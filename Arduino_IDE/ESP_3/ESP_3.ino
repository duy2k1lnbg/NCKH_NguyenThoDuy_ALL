...
void setup()
{
  ...
  server.on("/" ,HTTP_GET, [](AsyncWebServerRequest * request) 
    {
      request->send(200, "text/piain","Test_esp_so_3.");
    });
  
  AsyncElegantOTA.begin(&server, "Duy3", "123456");
  server.begin();
...
void loop()
{
  ...
  while(WiFi.status() == WL_CONNECTED)
  {
    ...
    if(Existence > 0)
    {
      ...
      int STATUS_1 = root["ID_QUAT_3"]["STATUS"];
      int STATUS_2 = root["ID_QUATTRAN_3"]["STATUS"];
      int STATUS_3 = root["ID_BONGDEN_3"]["STATUS"];
      int STATUS_4 = root["ID_DIEUHOA_3"]["STATUS"];
      Serial.println("========================");
